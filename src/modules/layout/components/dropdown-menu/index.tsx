...
import useCollections from "@lib/hooks/data-fetching/useCollections"
...
const DropdownMenu = () => {
  ...
  const { data: products, isLoading: loadingProducts } = useFilter(
    "category=collection"
  )
                    ...
                    <div className="flex-1">
                      <div className="grid grid-cols-3 gap-4">
                        {products?.slice(0, 3).map((p) => (
                          <ProductPreview product={p} key={p.id} />
                        ))}
                        {loadingProducts &&
                          repeat(3).map((index) => (
